...
ErrorCode createMergedArray(Node list1, Node list2, int **ptr_res_err, int* result_array_len) 
{
  ...
  int *ptr_len1 = &len1, *ptr_len2 = &len2, **ptr_array1 = &array1, **ptr_array2 = &array2;
  if (getNumsArrayFromList(list1, ptr_len1, ptr_array1) != SUCCESS) {
    return MEMORY_ERROR;
  }
  if (getNumsArrayFromList(list2, ptr_len2, ptr_array2) != SUCCESS) {
    free(array1);
    return MEMORY_ERROR;
  }
...
ErrorCode mergeSortedLists(Node list1, Node list2, Node *merged_out)
{
  ErrorCode res_code = validateLists(list1, list2, merged_out);
  if (res_code != SUCCESS) {
    return res_code;
  }
  *merged_out = NULL;
  int *merged_num_array, merged_array_len;
  int **ptr_merged_num_array= &merged_num_array, *ptr_merged_array_len = &merged_array_len;
  if (createMergedArray(list1, list2, ptr_merged_num_array, ptr_merged_array_len) != SUCCESS) {
    return MEMORY_ERROR;
  }
  res_code = createListByNumsArr(merged_out, merged_num_array, merged_array_len);
